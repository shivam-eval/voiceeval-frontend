    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-dark-panel border border-gray-800 rounded-xl shadow-2xl max-w-md w-full p-8 relative">
        ...
        <p className="text-gray-400 text-center mb-6">
          Test Suite:{" "}
          <span className="text-teal-400 font-mono">{testSuiteId}</span>
        </p>
...
const CreateInboundSessionModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Ensure the payload includes a metadata flag indicating inbound
      const payload = {
        ...formData,
        metadata: {
          ...(formData.metadata || {}),
      ...
      const result = await runInbound.mutateAsync(payload);
      ...
      const inboundNumber =
        selectedClient?.inbound_number || "the designated number";
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFormData({ ...formData, test_suite_id: e.target.value })
              }
              className="w-full bg-dark-input border border-gray-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-teal-500"
...
const InboundPage = () => {
  ...
  const inboundSimulations = useMemo(() => {
    ...
    return simulationsData.simulations.filter((sim) => {
      const callMode = (sim.call_mode || sim.metadata?.call_mode || "")
        .toString()
        .toLowerCase();
  ...
  const handleRowClick = async (sim) => {
    try {
      ...
      const simulationDetail = await simulationsService.getSimulation(
        sim.simulation_id,
      );
    ...
    <div className="p-8 bg-dark-bg min-h-screen text-white">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-400">
              View and manage all inbound simulation sessions
            </p>
          </div>
          <button
              ...
              <div className="flex flex-col items-center gap-4">
                ...
                <div>
                  <p className="text-gray-400 text-lg font-medium">
                    No inbound sessions found
                  </p>
                  <p className="text-gray-500 text-sm">
                    Create your first inbound simulation to see results here
                  </p>
                </div>
                <button
                  onClick={() => setCreateModal(true)}
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="text-white font-semibold text-sm">
                            {sim.metadata?.test_suite_name ||
                              sim.metadata?.flow_tree_name ||
                              sim.test_suite_id}
                          </span>
                        ...
                        <div className="flex flex-col gap-1.5 min-w-[120px]">
                          ...
                          <div className="w-full bg-gray-800/50 rounded-full h-1.5 overflow-hidden border border-gray-700/30">
                            <div
                              ...
                              style={{
                                width: `${sim.progress?.percentage || 0}%`,
                              }}
                            ></div>
                          </div>
                        </div>
                        ...
                        sim.progress?.total_test_cases > 0 ? (
                          (() => {
                            const total =
                              ...
                              sim.progress?.total_test_cases ||
                              0;
                            const completed = sim.progress?.completed || 0;
                            const percentage =
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          {sim.status === "running" ? (
                            <button
                              onClick={(e) =>
                                handleCancel(e, sim.simulation_id)
                              }
                            ...
                            <>
                              <button
                                onClick={(e) => {
                                  ...
                                  setConfirmModal({
                                    ...
                                    message:
                                      "This will create a new simulation with the same parameters. Continue?",
                                    confirmText: "Rerun",
                                    variant: "teal",
                                    onConfirm: async () => {
                                      try {
                                        const result =
                                          await rerunSimulation.mutateAsync(
                                            sim.simulation_id,
                                          );
                                        toast.success(
                                          `Simulation rerun initiated!`,
                                        );
                                        ...
                                        navigate(
                                          `/simulation/runs/${result.new_simulation_id}`,
                                        );
                                      } catch (error) {
                                        // handled globally
                                      }
                              ...
                              <button
                                onClick={(e) =>
                                  handleDelete(e, sim.simulation_id)
                                }
                                className="p-2 rounded hover:bg-gray-800 text-gray-400 hover:text-red-400 transition-colors"
                                title="Delete Simulation"
                              >
      ...
      <ConfirmationModal
        ...
        isLoading={
          ...
          cancelSimulation.isPending ||
          rerunSimulation.isPending
        }
      />
    </div>
  );
